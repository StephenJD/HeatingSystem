...
public:
	Label(int label, Behaviour behaviour = newLine()) : _label(label), _behaviour(behaviour) {}
	// Queries
...
TEST_CASE("Collection_RangeFor_non_const") {
	Label L0(0), L1(1), L2(2), L3(3), L4(4);
	auto page_c = makeCollection(L0, L1, L2, L3, L4);
	cout << "L0 addr:" << hex << (long)&L0 << endl;

	int index = 0;
	for (auto & i : page_c) { // i is assumed to be to be one of the collections pointed to by the page_c collection of Coll_Hdls.
		auto & thisInt = static_cast<Label &>(i);
		cout << "addr i: " << hex << (long)&i << " L0 addr:" << (long)&thisInt << endl;
		cout << "Collection_RangeFor : " << (int)thisInt << endl;
		CHECK(thisInt == index);
		thisInt = thisInt * 2;
		++index;
	}
...
TEST_CASE("Command Change Focus") {
	...
	element->get()->select(0); // calls page.MoveFocusTo()
	CHECK(page_h.focusIndex() == 1);
...
TEST_CASE("Select Active UI on Page") {
	...
	auto newRecipient = page1_h->select(0);
	//newRecipient->get()->select(0); // calls page.MoveFocusTo()
	//CHECK(display1_h.focusIndex() == 1);
	//CHECK(page2_h.focusIndex() == 1);

	////newRecipient = static_cast<Cmd &>(*page2_h.select(0));
	//newRecipient = page2_h.get()->select(0);
	//newRecipient->get()->select(0); // calls page.MoveFocusTo()
