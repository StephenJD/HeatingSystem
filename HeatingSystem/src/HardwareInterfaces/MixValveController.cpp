...
//#ifndef ZPSIM
		...
		if (algorithmMode >= Mix_Valve::e_Error) {
			logger() << L_time << (valveIndex == M_UpStrs ? "_US_Mix" : "_DS_Mix") << "\tMode Error: " << algorithmMode << L_endl;
			return false;
		}
...
//#endif
	...
	bool MixValveController::readRegistersFromValve_OK() { // called once per second
		...
		if (status == _OK) {
			...
			logger() << "give_I2C_Bus to RemOff: " << _remoteRegOffset << " LocalOff: " << _localRegOffset << L_endl;
