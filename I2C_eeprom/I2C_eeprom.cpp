...
I2C_eeprom::I2C_eeprom(const uint8_t deviceAddress, const unsigned int deviceSize, TwoWire & wire_port)
	: _wire_port(&wire_port), _lastWrite(0)
{
...
void I2C_eeprom::_beginTransmission(const uint16_t memoryAddress)
{
  ...
  {
	  _wire_port->write((memoryAddress >> 8));    // Address High Byte
  }

  _wire_port->write((memoryAddress & 0xFF));  // Address Low Byte (or only byte for chips 16K or smaller that only have one-word addresses)
}
...
int I2C_eeprom::_WriteBlock(const uint16_t memoryAddress, const uint8_t* buffer, const uint8_t length)
{
    ...
    _wire_port->write(buffer, length);
    int rv = _wire_port->endTransmission();
...
uint8_t I2C_eeprom::_ReadBlock(const uint16_t memoryAddress, uint8_t* buffer, const uint8_t length)
{
    ...
    uint8_t readBytes = _wire_port->requestFrom(_deviceAddress, length);
    uint8_t cnt = 0;
    while (cnt < readBytes)
    {
        buffer[cnt++] = _wire_port->read();
    }
    return readBytes;
}
