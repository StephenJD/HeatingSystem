...
public:
	...
	virtual uint8_t testDevice(I2C_Talk & i2c, int addr) { return 0; }
	uint8_t getAddress() const { return _address; }
	void setAddress(uint8_t addr) { _address = addr; }

	uint8_t read(uint8_t registerAddress, uint16_t numberBytes, uint8_t *dataBuffer) { return i2c_Talk().read(getAddress(), registerAddress, numberBytes, dataBuffer); } // Return errCode. dataBuffer may not be written to if read fails.
	...
	uint8_t write(uint8_t registerAddress, uint16_t numberBytes, const uint8_t *dataBuffer) { return i2c_Talk().write(getAddress(), registerAddress, numberBytes, dataBuffer); }  // Return errCode.
	uint8_t write(uint8_t registerAddress, uint8_t data) { return i2c_Talk().write(getAddress(), registerAddress, data); } // Return errCode.
	...
	void writeAtZeroCross() { i2c_Talk().writeAtZeroCross(); }
	uint8_t notExists() { return i2c_Talk().notExists(getAddress()); }
	// slave response
