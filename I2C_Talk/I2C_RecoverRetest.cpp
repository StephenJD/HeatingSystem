...
bool I2C_Recover_Retest::tryReadWriteAgain(uint8_t status) {
	return status && ++noOfFailures < noOfRetries && restart("read 0x") && g_delayTrue(noOfFailures); // create an increasing delay if we are going to loop
}
...
bool I2C_Recover_Retest::checkForTimeout(uint8_t status) {
...
uint8_t I2C_Recover_Retest::speedOK() {
	if (i2C().getThisI2CFrequency(addr()) == 0) {
		if (millis() - getFailedTime() > 600000) {
			i2C().setThisI2CFrequency(addr(), 400000);
		}
...
bool I2C_Recover_Retest::endTransmissionError(uint8_t error) {
...
bool I2C_Recover_Retest::slowdown_and_reset() { // called by check_endTransmissionOK only with auto-speed
	bool canReduce = false;
	if (millis() - getFailedTime() < 10000) { // within 10secs try reducing speed.
...
void I2C_Recover_Retest::callTime_OutFn() {
...
signed char I2C_Recover_Retest::testDevice(I_I2Cdevice * deviceFailTest, int noOfTestsMustPass) {
	signed char testFailed = 0;
	signed char failed;
	int noOfFailedTests = 0;
	do {
		failed = I2C_Recover::testDevice(deviceFailTest);
		if (failed) {
			++noOfFailedTests;
			testFailed |= failed;
		}
...
signed char I2C_Recover_Retest::findAworkingSpeed(I_I2Cdevice * deviceFailTest) {
	...
	int highFailed = testDevice(0, 1);
	if (!highFailed) highFailed = testDevice(deviceFailTest, 1);
	...
	if (testFailed) {
		for (auto freq : tryFreq) {
			...
			if (testFailed == I2C_Talk::_OK) {
				testFailed = testDevice(deviceFailTest, 1);
				if (testFailed == I2C_Talk::_OK) {
	...
	return testFailed ? I2C_Talk::_I2C_Device_Not_Found : I2C_Talk::_OK;
}
...
signed char I2C_Recover_Retest::adjustSpeedTillItWorksAgain(I_I2Cdevice * deviceFailTest, int32_t incrementRatio) {
	...
	do { // Adjust speed 'till it works reliably
		hasFailed = testDevice(deviceFailTest, noOfTestsMustPass);
		if (hasFailed) {
			...
			callTime_OutFn();
			if (hasFailed == I2C_Talk::_Timeout) callTime_OutFn();
		}
