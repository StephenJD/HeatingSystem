...
//#include <..\HeatingSystem\src\HardwareInterfaces\ConsoleController_Thick.h>
...
byte i2cAddr[] = { 0x10,0x12,0x13,0x14,0x20,0x28,0x29,0x2B,0x2D,0x2E,0x2F,0x35,0x36,0x37,0x48,0x4B,0x4F,0x70,0x71,0x74,0x75,0x76,0x77 };
...
TempSensor tempSens[] = { {i2c_recover, 0x28},0x29C,0x2B,0x2D,0x2E,0x2F,0x35,0x36,0x37,0x48,0x4B,0x4F,0x70,0x71,0x74,0x75,0x76,0x77 };
...
ConsoleController_Thick thickConsole_Arr[] = { {i2c_recover, 0x12},{i2c_recover, 0x13},{i2c_recover, 0x14} };
...
I_I2Cdevice_Recovery & getDevice(int addr) {
	if (addr == 0x10) {
		...
		return thickConsole_Arr[addr - 0x12];
	} else if (addr == 0x20) {
		return rPort;
	}	
...
uint8_t tryDeviceAt(int addrIndex) {
	...
	else if (addr < 0x20) { // Remote ThickDisplay
		hasFailed = device.testDevice();
