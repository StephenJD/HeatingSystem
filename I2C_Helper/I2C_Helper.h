...
This library takes an optional void timeoutFunction() as an argument to the setTimeoutFn method.
...
public:
	...
	static uint8_t status(I2C_Helper & i2c, int deviceAddr);
	uint8_t status(int deviceAddr);
	...
	bool slowdown_and_reset(int addr); // called by timeoutFunction. Reduces I2C speed by 10% if last called within one second. Returns true if speed was reduced
	...
	void setAsSlave(int slaveAddress) { _myAddress = slaveAddress; _canWrite = false; begin(); }
	void setAsMaster(int multiMaster_MyAddress = _single_master) {_myAddress = multiMaster_MyAddress; _canWrite = true; begin();}
...
private:
	bool begin(const char * name, int addr);
	...
	while(scan<false,false>()) {
		if (serial_out) {
			if (result.error == _OK) {
				...
				Serial.println(getStatusMsg(result.error));
			}
		}
