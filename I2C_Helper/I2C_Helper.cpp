...
uint8_t I2C_Helper::status(I2C_Helper & i2c, int deviceAddr) {
	return i2c.status(deviceAddr);
}

uint8_t I2C_Helper::status(int deviceAddr) {
...
uint8_t I2C_Helper::read(uint16_t deviceAddr, uint8_t registerAddress, uint16_t numberBytes, uint8_t *dataBuffer) {
	...
	} while (returnStatus && ++i < noOfRetries && begin("read 0x",deviceAddr) && g_delayTrue(i));// create an increasing delay if we are going to loop
...
uint8_t I2C_Helper::getData(uint16_t deviceAddr, uint16_t numberBytes, uint8_t *dataBuffer) {
	...
	if (returnStatus == _OK) {
		...
		if (returnStatus != _OK) { // returned error
			...
			//log("I2C_Helper::read err: for ", long(deviceAddr), getStatusMsg(returnStatus),long(i));
...
uint8_t I2C_Helper::write(uint16_t deviceAddr, uint8_t registerAddress, uint16_t numberBytes, const uint8_t * dataBuffer) {
	...
	do{
		...
		if (returnStatus == _OK) {
			...
			_lastWrite = micros();
			if (returnStatus) {
				//log(" I2C_Helper::write Error Writing addr: ",long(deviceAddr), getStatusMsg(returnStatus),long(i)); 
				//Serial.print(i,DEC); 
			...
			relayDelay = micros() - relayStart;	
		} else return returnStatus;
	} while (returnStatus && ++i < noOfRetries && begin("write 0x",deviceAddr) && g_delayTrue(i));// create an increasing delay if we are going to loop
	if (i < noOfRetries && i > successAfterRetries) successAfterRetries = i;
	return(returnStatus);
}
...
const char * I2C_Helper::getStatusMsg(int errorCode) {
	switch (errorCode) {
...
uint8_t I2C_Helper::check_endTransmissionOK(int addr) { // returns 0=OK, 1=Timeout, 2 = Error during send, 3= NACK during transmit, 4=NACK during complete.
	...
	//else if (error != _OK) {
		//Serial.print("check_endTransmissionOK() failure: "); Serial.println(getStatusMsg(error));
	//}
	return error;
}
...
bool I2C_Helper::slowdown_and_reset(int addr) { // called by check_endTransmissionOK only with auto-speed
	...
	if (millis() - getFailedTime(addr) < 10000) { // within 10secs try reducing speed.
		...
		if (canReduce) {
			//logger() << "slowdown_and_reset for" << addr << "speed was" << _i2cFreq << L_endl;
			setThisI2CFrequency(addr, _i2cFreq - _i2cFreq / 10);
			//logger() << "slowdown_and_reset for" << addr << "reduced speed to" << _i2cFreq << L_endl;
		}
		else {
			result.reset();
			result.foundDeviceAddr = addr;
			speedTest();
			//logger() << "slowdown_and_reset called speedTest. Now:" << _i2cFreq << L_endl;
		}
...
void I2C_Helper::waitForZeroCross(){
	...
	if (s_zeroCrossPin) { //////////// Zero Cross Detection function ////////////////////////
		auto zeroCrossSignalTime = micros();
		while (digitalRead(abs(s_zeroCrossPin)) == (s_zeroCrossPin>0) && int32_t(micros() - zeroCrossSignalTime) < HALF_MAINS_PERIOD); // We need non-active state.
		fireTime = micros();
		while (digitalRead(abs(s_zeroCrossPin)) == (s_zeroCrossPin<0) && int32_t(micros() - zeroCrossSignalTime) < HALF_MAINS_PERIOD); // Now wait for active state.
		zeroCrossSignalTime = micros();
		int16_t fireDelay = s_zxSigToXdelay - relayDelay;
		if (fireDelay < 0) fireDelay += HALF_MAINS_PERIOD;
		fireTime = fireTime + s_zxSigToXdelay - relayDelay;
		while (int32_t(micros() - zeroCrossSignalTime) < fireDelay); // wait for fireTime.
...
bool I2C_Helper::i2C_is_frozen(int addr) {
	...
	if (addr) {
		...
		if (micros() - timeOut > 20000) {
			//logger() << "****  i2C_is_frozen timed-out  ****" << L_endl;
			if (timeoutFunctor != 0) {
...
bool I2C_Helper::begin() {
	logger() << "I2C Restart..." << L_endl;
	wire_port.begin(_myAddress);
	return true;
}

bool I2C_Helper::begin(const char * name,int addr) {
	begin();
	//Serial.print("I2C::begin() "); Serial.print(name);Serial.println(addr, HEX);
	return true;
}
...
void I2C_Helper::callTime_OutFn(int addr) {
	...
	if (i2C_is_frozen(addr)) {
		if (timeoutFunctor != 0) {
			if (doingTimeOut) {
				//logger() << "callTime_OutFn called recursivly" << L_endl;
				return;
			}
			doingTimeOut = true;
			...
			begin(); // begin i2c in case this is called after an i2c failure
...
signed char I2C_Helper::testDevice(I_I2Cdevice * deviceFailTest, uint8_t addr, int noOfTestsMustPass) {
	...
	do {
		if (deviceFailTest == 0) {
			//Serial.println("testDevice - none set, using status");
			failed = status(*this, addr);
		}
		else {
...
signed char I2C_Helper::findAworkingSpeed(I_I2Cdevice * deviceFailTest) {
	...
	if (testFailed) {
		for (auto freq : tryFreq) {
			...
			//logger().log("Trying addr:", result.foundDeviceAddr,"at freq:",freq);// Serial.print(" Success: "); Serial.println(getStatusMsg(testFailed));
			auto timeOut = micros();
			testFailed = testDevice(0, result.foundDeviceAddr, 1);
			if (micros() - timeOut > 20000) {
				//logger() << "****  findAworkingSpeed timed-out   at freq:" << freq << L_endl;
				if (timeoutFunctor != 0) {
...
int I2C_Helper_Auto_Speed_Hoist::_findDevice(int16_t devAddr, int8_t * devAddrArr, int noOfDevices) {
	int index = 0;
	do {
		if (devAddrArr[index] == 0) { devAddrArr[index] = int8_t(devAddr); break; }
